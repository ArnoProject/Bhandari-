...
export default async function handler(req, res) {
  ...
  try {
    ...
    pg.drawText(invoice.clientName||'', { x:310, y, size:11, font:fontBold });
    y -= 14;
    pg.drawText('7615 N 90TH ST, Omaha NE 68122', { x:50, y, size:10, font });
    pg.drawText(invoice.clientAddress||'', { x:310, y, size:10, font });
    y -= 35;
    ...
    pg.drawText(`${invoice.origin||''} → ${invoice.dest||''} · Driver: ${invoice.driver||''}`, { x:58, y:y-14, size:9, font, color:rgb(.4,.4,.4) });
    ...
    y -= 35;
    for (const c of (invoice.extraCharges||[]).filter(x=>x.desc&&x.amount)) {
      pg.drawText(c.desc, { x:58, y, size:10, font });
      pg.drawText('1', { x:385, y, size:10, font });
      pg.drawText(fmtMoney(c.amount), { x:425, y, size:10, font });
    ...
    for (const rs of (rateSheets||[])) {
      ...
      } catch(e) { console.error('Rate sheet error:', e.message); }
    }
