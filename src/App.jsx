...
const fgrid3 = { display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 12, marginBottom: 14 };

const Field = ({ label, type = "text", value, onChange, options, span, placeholder, col3 }) => (
...
// ─── Form Components (all outside App to prevent re-render) ───────────────────
...
const MaintenanceForm = ({ onClose, onSave, saving, trucks, trailers, editItem }) => {
  ...
  const tirePositions = f.entity_type === "truck" ? TIRE_POSITIONS_TRUCK : TIRE_POSITIONS_TRAILER;
  const brakePositions = f.entity_type === "truck" ? BRAKE_POSITIONS_TRUCK : BRAKE_POSITIONS_TRAILER;
  return (
    <ModalShell title={editItem ? "✏️ Edit Service Record" : "🔧 Add Maintenance Record"} onClose={onClose} wide>
      <div style={fgrid}>
        ...
        {(isTire || isBrake) && <Field label={isTire ? "Tire Position" : "Brake Position"} value={f.position} onChange={v => setF(p => ({ ...p, position: v }))} options={isTire ? tirePositions : brakePositions} span />}
        ...
        <Field label="Notes" value={f.notes} onChange={v => setF(p => ({ ...p, notes: v }))} placeholder="Any additional notes..." span />
      </div>
...
const LoadForm = ({ onClose, onSave, saving, trucks, trailers, editItem }) => {
  const [f, setF] = useState(editItem || { date: today(), loadNum: "", origin: "", dest: "", miles: "", rate: "", detention: "0", driver: "", driverPct: "28", truckId: trucks[0]?.id || "", trailerId: "", status: "Pending", lumperCost: "0", lumperPaidBy: "Out of Pocket", lumperReimbursed: "No", lumperReimbursedAmount: "0", toll: "0" });
    ...
    <ModalShell title={editItem ? "✏️ Edit Load" : "🚛 Add Load"} onClose={onClose} wide>
      ...
      {f.rate && (
        <div style={{ background: "#f0fdf4", border: "1.5px solid #bbf7d0", borderRadius: 10, padding: "14px 16px", marginBottom: 14 }}>
          <div style={{ color: "#6b7280", fontSize: 10, fontWeight: 700, marginBottom: 10 }}>LIVE PREVIEW</div>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(5,1fr)", gap: 10, textAlign: "center" }}>
            {[{ l: "Gross", v: fmt$(g), c: "#16a34a" }, { l: "Driver Pay", v: fmt$(dp), c: "#d97706" }, { l: "Lumper (net)", v: fmt$(lumperNet), c: "#dc2626" }, { l: "Toll", v: fmt$(Number(f.toll||0)), c: "#7c3aed" }, { l: "Your Profit", v: fmt$(profit), c: profit >= 0 ? "#16a34a" : "#dc2626" }].map(s => (
...
const RateConModal = ({ onClose, onLoad, trucks, trailers }) => {
        ...
        <>
          ...
          {parsing ? (
            <div style={{ textAlign: "center", padding: "40px 0" }}>
              ...
              <div style={{ color: "#d97706", fontWeight: 800, fontSize: 18 }}>Reading your rate confirmation...</div>
            </div>
          ) : (
            <>
...
const MudflapModal = ({ onClose, onImport, saving, trucks }) => {
  ...
  const handleFile = async (e) => {
    ...
    try {
      ...
      const result = await parseWithAI(base64, mediaType, `Extract ALL fuel transactions from this Mudflap statement. Return ONLY a JSON array:\n[{"date":"YYYY-MM-DD","location":"station name and city","gallons":number,"amount":number,"saved":number}]\nIf you see dates like "Apr 03" convert to full date using the statement year. Return ONLY the JSON array, no other text.`);
      if (result && Array.isArray(result) && result.length > 0) {
        setTransactions(result);
        const m = {};
        result.forEach((_, i) => { m[i] = trucks[0]?.id || ""; });
        setMapping(m);
  ...
  const doImport = async () => {
    const rows = transactions.map((t, i) => ({ id: uid(), date: t.date, truck_id: mapping[i] || trucks[0]?.id, gallons: Number(t.gallons || 0), price_per: Number(t.gallons) > 0 ? Number(t.amount) / Number(t.gallons) : 0, total: Number(t.amount || 0), location: t.location || "", load_num: "" }));
    await onImport(rows);
    setDone(true);
  };
    ...
    <ModalShell title="🟠 Import Mudflap Statement" onClose={onClose} extraWide>
      {done ? (
        <div style={{ textAlign: "center", padding: "40px 0" }}>
          ...
          <div style={{ color: "#16a34a", fontWeight: 900, fontSize: 24 }}>Imported Successfully!</div>
          <PrimaryBtn onClick={onClose} style={{ marginTop: 20 }}>Done</PrimaryBtn>
        </div>
      ) : transactions.length === 0 ? (
        <>
          <div style={{ background: "#fff7ed", border: "1.5px solid #fed7aa", borderRadius: 10, padding: "14px 18px", marginBottom: 18 }}>
            <div style={{ color: "#c2410c", fontWeight: 700, marginBottom: 6 }}>How it works:</div>
            <div style={{ color: "#9a3412", fontSize: 13, lineHeight: 1.7 }}>Upload your Mudflap PDF statement → AI reads all fuel transactions → You assign each transaction to the correct truck → Import saves everything to your fuel log.</div>
          </div>
          {parsing ? (
            <div style={{ textAlign: "center", padding: "40px 0" }}>
              <div style={{ fontSize: 48, marginBottom: 16 }}>🤖</div>
              <div style={{ color: "#d97706", fontWeight: 800, fontSize: 18 }}>Reading your Mudflap statement...</div>
              <div style={{ color: "#6b7280", fontSize: 14, marginTop: 8 }}>AI is extracting transactions</div>
            </div>
          ) : (
            <>
              <div style={{ border: "2px dashed #d1d5db", borderRadius: 12, padding: 40, textAlign: "center", cursor: "pointer", background: "#f9fafb" }} onClick={() => fileRef.current?.click()} onMouseEnter={e => e.currentTarget.style.borderColor = "#ea580c"} onMouseLeave={e => e.currentTarget.style.borderColor = "#d1d5db"}>
                <div style={{ fontSize: 48, marginBottom: 12 }}>🟠</div>
                <div style={{ color: "#374151", fontSize: 16, fontWeight: 700, marginBottom: 6 }}>Click to upload Mudflap PDF Statement</div>
                <div style={{ color: "#9ca3af", fontSize: 13 }}>Download from mudflap.com → Statements</div>
        ...
        <>
          <div style={{ background: "#f0fdf4", border: "1.5px solid #bbf7d0", borderRadius: 10, padding: "12px 16px", marginBottom: 18 }}>
            <div style={{ color: "#166534", fontWeight: 700 }}>✅ {transactions.length} transactions found — assign each to a truck:</div>
          </div>
              ...
              <tbody>
                {transactions.map((t, i) => (
                  <tr key={i} onMouseEnter={e => e.currentTarget.style.background = "#f9fafb"} onMouseLeave={e => e.currentTarget.style.background = "#fff"}>
                    <TD>{t.date}</TD>
                    <TD>{t.location}</TD>
                    <TD mono>{fmtN(t.gallons, 1)}</TD>
                    <TD mono color="#dc2626" bold>{fmt$(t.amount)}</TD>
                    <TD mono color="#16a34a">{fmt$(t.saved)}</TD>
                    <TD>
                      <select value={mapping[i] || ""} onChange={e => setMapping(p => ({ ...p, [i]: e.target.value }))} style={{ ...inputStyle, width: "auto", minWidth: 150 }}>
  ...
  return (
    <ModalShell title={editItem ? "✏️ Edit Insurance" : "🛡️ Add Insurance Policy"} onClose={onClose} wide>
      <div style={fgrid}>
        ...
        <Field label="Premium Amount ($)" type="number" value={f.premium_amount || ""} onChange={v => setF(p => ({ ...p, premium_amount: v }))} placeholder="0.00" />
        ...
        <Field label="Notes" value={f.notes || ""} onChange={v => setF(p => ({ ...p, notes: v }))} span placeholder="Any additional notes..." />
      </div>
...
export default function App() {
  ...
  const fetchAll = async () => {
    ...
    try {
      ...
      if (l.data) setLoads(l.data.map(r => ({ id: r.id, date: r.date, loadNum: r.load_num, origin: r.origin, dest: r.dest, miles: r.miles, rate: r.rate, detention: r.detention, driver: r.driver, driverPct: r.driver_pct, truckId: r.truck_id, trailerId: r.trailer_id, status: r.status, lumperCost: r.lumper_cost, lumperPaidBy: r.lumper_paid_by, lumperReimbursed: r.lumper_reimbursed, lumperReimbursedAmount: r.lumper_reimbursed_amount, toll: r.toll })));
      ...
      if (ins.data) setInsurance(ins.data.map(r => ({ id: r.id, provider: r.provider, policyNumber: r.policy_number, coverageType: r.coverage_type, premiumAmount: r.premium_amount, paymentFrequency: r.payment_frequency, startDate: r.start_date, endDate: r.end_date, entityId: r.entity_id, entityType: r.entity_type, notes: r.notes })));
  ...
  const saveTrailer = async (f) => { if (!f.name) return; setSaving(true); const row = { id: editItem?.id || ("TR" + uid()), name: f.name, plate: f.plate || "", year: f.year || "", make: f.make || "Utility", model: f.model || "", type: f.type || "Reefer", color: f.color || TRUCK_COLORS[2].value, active: f.active !== false }; const { error } = await db.from("trailers").upsert(row); if (error) showToast("Save failed", "error"); else { await fetchAll(); closeModal(); showToast(editItem ? "Trailer updated ✓" : "Trailer added ✓"); } setSaving(false); };

  const saveLoad = async (f) => {
    if (!f.loadNum || !f.rate) return;
    setSaving(true);
    const row = { id: editItem?.id || uid(), date: f.date, load_num: f.loadNum, origin: f.origin, dest: f.dest, miles: Number(f.miles || 0), rate: Number(f.rate), detention: Number(f.detention || 0), driver: f.driver, driver_pct: Number(f.driverPct || 28), truck_id: f.truckId, trailer_id: f.trailerId || null, status: f.status, lumper_cost: Number(f.lumperCost || 0), lumper_paid_by: f.lumperPaidBy || "Out of Pocket", lumper_reimbursed: f.lumperReimbursed || "No", lumper_reimbursed_amount: Number(f.lumperReimbursedAmount || 0), toll: Number(f.toll || 0) };
    const { error } = await db.from("loads").upsert(row);
    if (error) showToast("Save failed", "error"); else { await fetchAll(); closeModal(); showToast(editItem ? "Load updated ✓" : "Load added ✓"); }
    setSaving(false);
  };

  const saveLoadDirect = async (f) => { if (!f.loadNum || !f.rate) { showToast("Missing load # or rate", "error"); return; } setSaving(true); const row = { id: uid(), date: f.date, load_num: f.loadNum, origin: f.origin, dest: f.dest, miles: Number(f.miles || 0), rate: Number(f.rate), detention: Number(f.detention || 0), driver: f.driver || "", driver_pct: Number(f.driverPct || 28), truck_id: f.truckId || trucks[0]?.id || "", trailer_id: f.trailerId || null, status: f.status || "Pending", lumper_cost: 0, lumper_paid_by: "Out of Pocket", lumper_reimbursed: "No", lumper_reimbursed_amount: 0, toll: 0 }; const { error } = await db.from("loads").upsert(row); if (error) showToast("Save failed", "error"); else { await fetchAll(); showToast("Load saved from rate con ✓"); } setSaving(false); };
  ...
  const saveMaintenance = async (f) => {
    if (!f.entity_id || !f.category) return;
    setSaving(true);
    const row = { id: editItem?.id || uid(), entity_id: f.entity_id, entity_type: f.entity_type, category: f.category, description: f.description || "", date: f.date, miles_at_service: Number(f.miles_at_service || 0), next_due_miles: Number(f.next_due_miles || 0), next_due_date: f.next_due_date || null, cost: Number(f.cost || 0), position: f.position || "", notes: f.notes || "" };
    const { error } = await db.from("maintenance").upsert(row);
    if (error) showToast("Save failed", "error");
    else {
      if (Number(f.cost) > 0) {
        const entityName = f.entity_type === "truck" ? trucks.find(t => t.id === f.entity_id)?.name : trailers.find(t => t.id === f.entity_id)?.name;
        await db.from("expenses").insert({ id: uid(), date: f.date, truck_id: f.entity_type === "truck" ? f.entity_id : "FLEET", category: "Maintenance", description: `${f.category}${f.position ? ` - ${f.position}` : ""} (${entityName})`, amount: Number(f.cost) });
      }
  ...
  const saveInsurance = async (f) => { if (!f.provider || !f.premium_amount) return; setSaving(true); const row = { id: editItem?.id || uid(), provider: f.provider, policy_number: f.policy_number || "", coverage_type: f.coverage_type, premium_amount: Number(f.premium_amount), payment_frequency: f.payment_frequency, start_date: f.start_date, end_date: f.end_date || null, entity_id: f.entity_id, entity_type: f.entity_type || "fleet", notes: f.notes || "" }; const { error } = await db.from("insurance").upsert(row); if (error) showToast("Save failed", "error"); else { await fetchAll(); closeModal(); showToast("Insurance policy saved ✓"); } setSaving(false); };
  ...
  // Maintenance alerts
  const maintAlerts = maintenance.filter(m => {
    if (!m.nextDueDate && !m.nextDueMiles) return false;
  ...
  const Dashboard = () => (
    <>
      <div style={S.ph}><div><h1 style={S.h1}>Dashboard</h1><div style={{ color: "#6b7280", fontSize: 12, marginTop: 3 }}>Bhandari Logistics LLC</div></div><button onClick={fetchAll} style={{ background: "#fff", border: "1.5px solid #d1d5db", borderRadius: 9, color: "#6b7280", padding: "9px 16px", cursor: "pointer", fontSize: 13, fontWeight: 600 }}>🔄 Refresh</button></div>
      <TruckBar />
      {(maintAlerts.length > 0 || insAlerts.length > 0) && (
        <div style={{ marginBottom: 18 }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill,minmax(280px,1fr))", gap: 10 }}>
            {maintAlerts.slice(0, 4).map(m => { const days = m.nextDueDate ? Math.ceil((new Date(m.nextDueDate) - new Date()) / 86400000) : null; const entity = m.entityType === "truck" ? trucks.find(t => t.id === m.entityId) : trailers.find(t => t.id === m.entityId); return (<AlertCard key={m.id} color={days <= 7 ? "#dc2626" : "#d97706"} icon={days <= 7 ? "🔴" : "🟡"} title={`${m.category} due — ${entity?.name || "Unknown"}`} sub={days <= 0 ? "OVERDUE!" : `Due in ${days} days (${m.nextDueDate})`} />); })}
            {insAlerts.slice(0, 2).map(i => { const days = Math.ceil((new Date(i.endDate) - new Date()) / 86400000); return (<AlertCard key={i.id} color="#7c3aed" icon="🛡️" title={`${i.provider} — ${i.coverageType} expiring`} sub={`Expires in ${days} days (${i.endDate})`} />); })}
          </div>
        </div>
      )}
      <div style={S.grid(5)}>
      ...
      {trucks.length === 0 ? (
        <div style={{ ...S.card, textAlign: "center", padding: "48px 24px" }}>
          ...
          <div style={{ color: "#111827", fontWeight: 800, fontSize: 20, marginBottom: 8 }}>Welcome to Bhandari!</div>
          <div style={{ color: "#6b7280", fontSize: 14, marginBottom: 24 }}>Start by adding your trucks and trailers.</div>
        ...
        <>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill,minmax(200px,1fr))", gap: 12 }}>
              {truckSummaries.map(t => (
                <div key={t.id} onClick={() => setTruckView(t.id)} style={{ background: "#f9fafb", border: `2px solid ${t.color}`, borderRadius: 11, padding: "14px 16px", cursor: "pointer" }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 10 }}><div style={{ width: 10, height: 10, borderRadius: "50%", background: t.color }} /><span style={{ color: "#111827", fontWeight: 800, fontSize: 14 }}>{t.name}</span><span style={{ color: "#9ca3af", fontSize: 11, marginLeft: "auto" }}>{t.numLoads}</span></div>
                  ...
                  {t.lastMaint && <div style={{ color: "#9ca3af", fontSize: 10, marginTop: 6 }}>Last service: {t.lastMaint.category} ({t.lastMaint.date})</div>}
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 18 }}>
            ...
            <div style={S.card}>
              ...
              {loads.slice(0, 6).map(l => { const g = Number(l.rate || 0) + Number(l.detention || 0); const truck = truckById(l.truckId); return (<div key={l.id} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "8px 0", borderBottom: "1px solid #f3f4f6" }}><div><div style={{ color: "#d97706", fontWeight: 700, fontSize: 13 }}>{l.loadNum}</div><div style={{ color: "#9ca3af", fontSize: 11 }}>{l.origin} → {l.dest}</div>{truck && <Badge label={truck.name} color={truck.color} />}</div><div style={{ textAlign: "right" }}><div style={{ color: "#16a34a", fontFamily: "monospace", fontWeight: 700 }}>{fmt$(g)}</div><StatusBadge s={l.status} /></div></div>); })}
            </div>
          </div>
        </>
      )}
  ...
  const Loads = () => (
    <>
      ...
      <TruckBar />
      <div style={S.grid(5)}><StatCard label="In Transit" value={filtLoads.filter(l => l.status === "In Transit").length} accent="#d97706" icon="🚛" /><StatCard label="Pending" value={filtLoads.filter(l => l.status === "Pending").length} accent="#6b7280" icon="⏳" /><StatCard label="Delivered" value={filtLoads.filter(l => l.status === "Delivered").length} accent="#16a34a" icon="✅" /><StatCard label="Detention" value={fmt$(totalDetain)} accent="#7c3aed" icon="⏱️" /><StatCard label="Total Rev" value={fmt$(totalRev)} accent="#16a34a" icon="💰" /></div>
      <div style={S.tableWrap}><div style={{ overflowX: "auto" }}><table style={{ width: "100%", borderCollapse: "collapse" }}><thead><tr>{["Load#", "Date", "Truck", "Trailer", "Origin → Dest", "Miles", "Rate", "Lumper", "Driver", "Profit", "$/mi", "Status", ""].map(h => <TH key={h}>{h}</TH>)}</tr></thead><tbody>
        ...
        {filtLoads.map(l => {
          ...
          const dp = g * (Number(l.driverPct || 0) / 100);
          ...
          return (<tr key={l.id} onMouseEnter={e => e.currentTarget.style.background = "#f9fafb"} onMouseLeave={e => e.currentTarget.style.background = "#fff"}>
            <TD color="#d97706" bold>{l.loadNum}</TD><TD>{l.date}</TD>
            ...
            <TD mono>${fmtN(Number(l.miles) ? g / Number(l.miles) : 0)}</TD>
            <TD><StatusBadge s={l.status} /></TD>
            <TD><div style={{ display: "flex", gap: 5 }}><button style={S.btnEdt} onClick={() => { setEditItem(l); setModal("load"); }}>Edit</button><button style={S.btnDel} onClick={() => delLoad(l.id)}>Del</button></div></TD>
          </tr>);
        })}
      </tbody></table></div></div>
    </>
  ...
  const Maintenance = () => {
    ...
    return (
      <>
        <div style={S.ph}><div><h1 style={S.h1}>Maintenance Tracker</h1><div style={{ color: "#6b7280", fontSize: 12, marginTop: 3 }}>Track service records for trucks & trailers — costs auto-log to expenses</div></div><PrimaryBtn onClick={() => { setEditItem(null); setModal("maintenance"); }}>+ Add Service Record</PrimaryBtn></div>
        <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginBottom: 20 }}>
        ...
        {maintAlerts.length > 0 && (
          <div style={{ marginBottom: 18 }}>
            <div style={{ color: "#6b7280", fontSize: 11, fontWeight: 700, letterSpacing: 1, marginBottom: 10, textTransform: "uppercase" }}>⚠️ Due Soon / Overdue</div>
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill,minmax(280px,1fr))", gap: 10, marginBottom: 14 }}>
              {maintAlerts.map(m => { const days = m.nextDueDate ? Math.ceil((new Date(m.nextDueDate) - new Date()) / 86400000) : null; const entity = m.entityType === "truck" ? trucks.find(t => t.id === m.entityId) : trailers.find(t => t.id === m.entityId); return (<AlertCard key={m.id} color={days <= 0 ? "#dc2626" : days <= 7 ? "#dc2626" : "#d97706"} icon={days <= 0 ? "🔴" : "🟡"} title={`${m.category}${m.position ? ` — ${m.position}` : ""} — ${entity?.name}`} sub={days <= 0 ? `OVERDUE by ${Math.abs(days)} days` : `Due in ${days} days (${m.nextDueDate})`} />); })}
            </div>
          </div>
        )}
        <div style={S.tableWrap}><div style={{ overflowX: "auto" }}><table style={{ width: "100%", borderCollapse: "collapse" }}><thead><tr>{["Date", "Equipment", "Category", "Position", "Miles", "Next Due Miles", "Next Due Date", "Cost", "Notes", ""].map(h => <TH key={h}>{h}</TH>)}</tr></thead><tbody>
          ...
          {filtered.map(m => {
            ...
            return (<tr key={m.id} onMouseEnter={e => e.currentTarget.style.background = "#f9fafb"} onMouseLeave={e => e.currentTarget.style.background = "#fff"}>
              <TD>{m.date}</TD>
              <TD>{entity && <Badge label={entity.name} color={entity.color || "#16a34a"} />}</TD>
              <TD bold color="#374151">{m.category}</TD>
              <TD color="#6b7280">{m.position || "—"}</TD>
  ...
  const Fleet = () => (
    <>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill,minmax(280px,1fr))", gap: 18, marginBottom: 28 }}>
        {truckSummaries.map(t => (
          <div key={t.id} style={{ background: "#fff", border: `2px solid ${t.color}`, borderRadius: 14, padding: 20, boxShadow: "0 2px 8px rgba(0,0,0,0.06)" }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 14 }}>
              <div><div style={{ color: t.color, fontWeight: 900, fontSize: 18 }}>{t.name}</div><div style={{ color: "#6b7280", fontSize: 12, marginTop: 2 }}>{[t.year, t.make, t.model].filter(Boolean).join(" ") || "No details"}</div>{t.plate && <div style={{ color: "#9ca3af", fontSize: 11 }}>{t.plate}</div>}</div>
              <div style={{ display: "flex", gap: 6 }}><button style={S.btnEdt} onClick={() => { setEditItem(t); setModal("truck"); }}>Edit</button><Badge label={t.active ? "Active" : "Inactive"} color={t.active ? "#16a34a" : "#dc2626"} /></div>
            </div>
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
              {[{ l: "Revenue", v: fmt$(t.rev), c: "#16a34a" }, { l: "Profit", v: fmt$(t.profit), c: t.profit >= 0 ? "#16a34a" : "#dc2626" }, { l: "Fuel", v: fmt$(t.fuel), c: "#dc2626" }, { l: "Driver Pay", v: fmt$(t.dp), c: "#d97706" }, { l: "Miles", v: fmtMi(t.mi), c: "#2563eb" }, { l: "$/Mile", v: "$" + fmtN(t.rpm), c: "#d97706" }].map(r => (<div key={r.l} style={{ background: "#f9fafb", borderRadius: 8, padding: "8px 10px", border: "1px solid #e5e7eb" }}><div style={{ color: "#6b7280", fontSize: 10, fontWeight: 700, textTransform: "uppercase" }}>{r.l}</div><div style={{ color: r.c, fontFamily: "monospace", fontWeight: 700, fontSize: 13, marginTop: 2 }}>{r.v}</div></div>))}
            </div>
            {t.lastMaint && <div style={{ background: "#fffbeb", border: "1px solid #fde68a", borderRadius: 8, padding: "8px 10px", marginTop: 10, fontSize: 12, color: "#92400e" }}>🔧 Last: {t.lastMaint.category} on {t.lastMaint.date}</div>}
            <div style={{ marginTop: 12, background: "#e5e7eb", borderRadius: 99, height: 5 }}><div style={{ width: `${Math.min(100, Math.max(0, t.margin))}%`, height: "100%", background: t.color, borderRadius: 99 }} /></div>
            <div style={{ color: "#6b7280", fontSize: 10, marginTop: 3 }}>{fmtN(t.margin)}% margin</div>
          </div>
        ))}
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill,minmax(260px,1fr))", gap: 16 }}>
        {trailers.map(t => {
          const tMaint = maintenance.filter(m => m.entityId === t.id).sort((a, b) => new Date(b.date) - new Date(a.date))[0];
          return (
            <div key={t.id} style={{ background: "#fff", border: `2px solid ${t.color || "#16a34a"}`, borderRadius: 14, padding: 20, boxShadow: "0 2px 8px rgba(0,0,0,0.06)" }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 14 }}>
                <div><div style={{ color: t.color || "#16a34a", fontWeight: 900, fontSize: 18 }}>🚛 {t.name}</div><div style={{ color: "#6b7280", fontSize: 12, marginTop: 2 }}>{t.type} · {[t.year, t.make, t.model].filter(Boolean).join(" ") || "No details"}</div>{t.plate && <div style={{ color: "#9ca3af", fontSize: 11 }}>{t.plate}</div>}</div>
                <div style={{ display: "flex", gap: 6 }}><button style={S.btnEdt} onClick={() => { setEditItem(t); setModal("trailer"); }}>Edit</button><Badge label={t.active ? "Active" : "Inactive"} color={t.active ? "#16a34a" : "#dc2626"} /></div>
              </div>
              {tMaint ? <div style={{ background: "#fffbeb", border: "1px solid #fde68a", borderRadius: 8, padding: "8px 10px", fontSize: 12, color: "#92400e" }}>🔧 Last: {tMaint.category} on {tMaint.date}</div> : <div style={{ color: "#9ca3af", fontSize: 12 }}>No service records yet</div>}
              <button onClick={() => { setEditItem(null); setModal("maintenance"); }} style={{ marginTop: 12, background: "#f0fdf4", color: "#16a34a", border: "1px solid #bbf7d0", borderRadius: 8, padding: "7px 14px", cursor: "pointer", fontSize: 12, fontWeight: 700, width: "100%" }}>+ Add Service Record</button>
            </div>
          );
        })}
  ...
  const Insurance = () => {
    ...
    return (
      <>
        ...
        <div style={S.grid(3)}>
          <StatCard label="Total Policies" value={insurance.length} accent="#7c3aed" icon="🛡️" />
          <StatCard label="Monthly Cost" value={fmt$(totalMonthly)} sub="Estimated" accent="#dc2626" icon="💰" />
          <StatCard label="Annual Cost" value={fmt$(totalMonthly * 12)} sub="Projected" accent="#d97706" icon="📅" />
        </div>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill,minmax(300px,1fr))", gap: 16 }}>
          {insurance.map(i => {
            const days = i.endDate ? Math.ceil((new Date(i.endDate) - new Date()) / 86400000) : null;
            return (
              <div key={i.id} style={{ background: "#fff", border: `1.5px solid ${days !== null && days <= 30 ? "#7c3aed" : "#e5e7eb"}`, borderRadius: 13, padding: 20, boxShadow: "0 1px 4px rgba(0,0,0,0.05)" }}>
                <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 12 }}>
                  <div><div style={{ color: "#7c3aed", fontWeight: 900, fontSize: 16 }}>🛡️ {i.provider}</div><div style={{ color: "#6b7280", fontSize: 12 }}>{i.coverageType}</div>{i.policyNumber && <div style={{ color: "#9ca3af", fontSize: 11 }}>#{i.policyNumber}</div>}</div>
                  <div style={{ display: "flex", gap: 6 }}><button style={S.btnEdt} onClick={() => { setEditItem(i); setModal("insurance"); }}>Edit</button><button style={S.btnDel} onClick={() => delInsurance(i.id)}>Del</button></div>
                </div>
                {[{ l: "Premium", v: fmt$(i.premiumAmount), c: "#dc2626" }, { l: "Frequency", v: i.paymentFrequency, c: "#374151" }, { l: "Start Date", v: i.startDate || "—", c: "#374151" }, { l: "Expiration", v: i.endDate || "—", c: days !== null && days <= 30 ? "#7c3aed" : "#374151" }].map(r => (<div key={r.l} style={{ display: "flex", justifyContent: "space-between", padding: "6px 0", borderBottom: "1px solid #f3f4f6" }}><span style={{ color: "#6b7280", fontSize: 12 }}>{r.l}</span><span style={{ color: r.c, fontWeight: 600, fontSize: 12 }}>{r.v}</span></div>))}
                {days !== null && days <= 60 && <div style={{ background: "#faf5ff", border: "1px solid #d8b4fe", borderRadius: 7, padding: "6px 10px", marginTop: 8, fontSize: 12, color: "#7c3aed", fontWeight: 700 }}>⚠️ Expires in {days} days</div>}
              </div>
            );
          })}
  ...
  const Drivers = () => (
    <>
      <div style={S.ph}><div><h1 style={S.h1}>Drivers & Paystubs</h1><div style={{ color: "#6b7280", fontSize: 12, marginTop: 3 }}>Drivers appear automatically when you add loads with a driver name</div></div><PrimaryBtn onClick={() => { setEditItem(null); setModal("load"); }}>+ Add Load with Driver</PrimaryBtn></div>
    ...
    <div style={S.app}>
      <div style={S.sidebar}>
        ...
        <div style={{ margin: "12px 12px 8px", background: "rgba(255,255,255,0.07)", borderRadius: 10, padding: "12px 14px" }}>
          ...
          {maintAlerts.length > 0 && <div style={{ background: "#dc262620", border: "1px solid #dc262644", borderRadius: 6, padding: "4px 8px", marginTop: 8, color: "#f87171", fontSize: 10, fontWeight: 700 }}>⚠️ {maintAlerts.length} maintenance alert{maintAlerts.length > 1 ? "s" : ""}</div>}
        </div>
        <div style={{ flex: 1 }}>
          {NAV.map(n => (<button key={n.id} style={S.navBtn(tab === n.id)} onClick={() => setTab(n.id)}><span style={{ fontSize: 15 }}>{n.icon}</span>{n.label}</button>))}
        </div>
        ...
        <div style={{ padding: "8px 12px", borderTop: "1px solid rgba(255,255,255,0.1)" }}>
          <div style={{ color: "#475569", fontSize: 9, letterSpacing: 1, marginBottom: 3 }}>☁️ SYNCED TO CLOUD</div>
          <div style={{ color: "#4ade8033", fontSize: 10 }}>Auto-saves · {trucks.length} trucks · {trailers.length} trailers</div>
        </div>
      </div>
